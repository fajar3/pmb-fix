...
class UserController extends Controller
{
    ...
    public function payment()
    {
        ...
        // Ambil pembayaran berdasarkan gender
        $payment = ($student->gender == 'Laki-laki') 
            ? PaymentPutra::where('user_id', $user->id)->first()
            : PaymentPutri::where('user_id', $user->id)->first();
    ...
    public function uploadPayment(Request $request)
    {
        ...
        // Simpan berdasarkan gender siswa
        if ($student->gender == 'Laki-laki') {
            PaymentPutra::create([
                ...
                'status' => 'pending',
                'nominal' => $request->nominal ?? 0, // Default 0 jika tidak diisi
            ]);
        } else {
            PaymentPutri::create([
                'user_id' => $user->id,
